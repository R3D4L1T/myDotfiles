...
function setTarget(){
        if [ $# -eq 1 ]; then
        	echo $1 > ~/.config/polybar/scripts/target
        elif [ $# -gt 2 ]; then
        	echo "settarget [IP] [NAME] | settarget [IP]"
        else
        	echo $1 $2 > ~/.config/polybar/scripts/target
        fi
}


# working location move
function setWork(){
  echo $(pwd) > ~/scripts/work_location
}

function goWork(){
  if [ -f ~/scripts/work_location ]; then
    cd "$(cat ~/scripts/work_location)"
  else
    echo "No work location set. Use 'setWork' first."
  fi
}
...
alias musescore="setsid /opt/museScore/MuseScore-Studio-4.5.2.251141401-x86_64.AppImage >/dev/null 2>&1 < /dev/null &"
